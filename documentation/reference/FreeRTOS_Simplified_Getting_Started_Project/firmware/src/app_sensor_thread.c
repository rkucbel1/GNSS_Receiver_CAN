...
void APP_SENSOR_THREAD_Initialize(void) 
{
    ...
    eventQueue = xQueueCreate( 1, sizeof(EVENT_INFO) );
    if (eventQueue == NULL)
    {
...
void APP_SENSOR_THREAD_Tasks(void) 
{
    ...
    {   
        ...
        /* Use FreeRTOS Queue to notify the temperature write event and temperature value to PRINT thread. */                                                  
        xQueueSend(eventQueue, (void*) &app_sensorData.eventInfo, 0);

        /*Use xQueueOverwrite instead of xQueueSend, to overwrite the data in the queue */
        //xQueueOverwrite(eventQueue, (void*) &app_sensorData.eventInfo);
    }        

    vTaskDelay(APP_SENSOR_SAMPLING_RATE_IN_MSEC / portTICK_PERIOD_MS); //1 second
    
}// end of temperature task
